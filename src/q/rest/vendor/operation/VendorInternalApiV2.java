...
public class VendorInternalApiV2 {
    ...
    public Response getSearchKeywordsDate(){
        try{
            String sql = "select z.* from (select cast(created as date), count(*) from vnd_search_keyword group by cast(created as date) order by cast(created as date) desc) z order by created desc";
